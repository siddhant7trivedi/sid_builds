...
- **Model**: hardcoded to `llama3.1:8b` via Ollama's local API (`http://localhost:11434/api/generate` or the `ollama` Python client). Do not build a model-abstraction layer for this — that's premature for a single fixed-model agent. Revisit abstraction only when a second agent needs to switch models at runtime.
- **Single call, structured output**: one prompt, one response, split into two clearly labeled sections (Jungian / Modern Psychology). JSON output preferred for reliable parsing into the UI.
- **Tone constraint — enforce explicitly in the prompt**: reflective and interpretive language only. Never phrase output as a diagnosis or definitive claim (avoid "this means," "you have," "this proves"). Use hedged framing throughout ("this may suggest," "one interpretation is," "this could reflect").
- **No fabrication of specificity**: if the dream description is very short/vague, the agent should give a shorter, appropriately general analysis rather than inventing elaborate detail to sound thorough.
- **No persistence**: do not build a database, file storage, or session history for v1. Input → output, nothing saved.
- **Input handling**: plain text area for the dream description.
- **Output UI**: two clearly separated sections (Jungian / Modern Psychology), rendered after submission.
